...
namespace valgo {
	Image::Image(std::string path, std::string name) : _path(path), _name(name), _width(0), _height(0), _angle(0), _right_shift(0), _top_shift(0) {}

	std::unique_ptr<SlideElement> Image::clone() const {
	...
	LatexCode Image::draw_as_latex() const {
		...
		ret << "{\\includegraphics[height=" << _height << "cm, width=" << _width << "cm, angle = " << _angle << "]{" << _name <<"}};\n";
		return ret.str();
	}

	HTMLCode Image::draw_as_html() const {
