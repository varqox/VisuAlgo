...
void Geometry::erase(GeometryElement& geo_elem) { _geo_elems.erase(&geo_elem); }

void Geometry::remove_all_elements() { _geo_elems.clear(); }
...
void Geometry::unset_coord_system() { _coordinate_system = false; }

std::unique_ptr<SlideElement> Geometry::clone() const { return std::make_unique<Geometry>(*this); }

HTMLCode Geometry::draw_as_html() const { throw NotImplemented(); }
