...
Block& Block::operator=(const Block& elem) { return (*this = Block(elem)); }

std::unique_ptr<SlideElement> Block::clone() const { return std::make_unique<Block>(*this); }
...
LatexCode Block::draw_as_latex() const {
	auto res =
	  concat("\\begin{colorblock}{", color_.to_hex(), "}{", contrast_color(color_).to_hex(), "}{", title_, "}\n");
	for (auto const& elem : elems_)
		back_insert(res, elem->draw_as_latex(), '\n');
	return back_insert(res, "\\end{colorblock}");
}

HTMLCode Block::draw_as_html() const { throw NotImplemented(); }
