...
protected:
	...
	struct Edge {
		...
		virtual DotCode to_dot(const std::map<NodeId, Node>& nodes, std::optional<char> side_from,
		                       std::optional<char> side_to, const std::vector<std::string>& opts = {},
...
public:
	...
	virtual Graph& add_edge(const NodeId& from, const NodeId& to,
	                        std::optional<EdgeInfo> edge_info = std::nullopt) = 0;
...
template <class NodeId, class NodeInfo, class EdgeInfo>
inline DotCode
Graph<NodeId, NodeInfo, EdgeInfo>::Edge::to_dot(const std::map<NodeId, Node>& nodes, std::optional<char> side_from,
                                                std::optional<char> side_to, const std::vector<std::string>& opts,
                                                bool reversed) const {
	std::stringstream ss;
...
template <class NodeId, class NodeInfo, class EdgeInfo>
inline Graph<NodeId, NodeInfo, EdgeInfo>&
Graph<NodeId, NodeInfo, EdgeInfo>::set_edge_color(const NodeId& from, const NodeId& to, std::optional<Color> color) {
	auto it = find_edge(from, to);
	if (it != edges_.end())
		it->second.color_ = std::move(color);
	return *this;
...
template <class NodeId, class NodeInfo, class EdgeInfo>
inline Graph<NodeId, NodeInfo, EdgeInfo>& Graph<NodeId, NodeInfo, EdgeInfo>::hide_node(const NodeId& node) {
	auto it = nodes_.find(node);
	if (it != nodes_.end())
...
template <class NodeId, class NodeInfo, class EdgeInfo>
inline Graph<NodeId, NodeInfo, EdgeInfo>& Graph<NodeId, NodeInfo, EdgeInfo>::hide_edge(const NodeId& from,
                                                                                       const NodeId& to) {
	auto it = find_edge(from, to);
	if (it != edges_.end())
		it->second.hidden_ = true;
...
template <class NodeId, class NodeInfo, class EdgeInfo>
inline Graph<NodeId, NodeInfo, EdgeInfo>& Graph<NodeId, NodeInfo, EdgeInfo>::unhide_node(const NodeId& node) {
	auto it = nodes_.find(node);
	if (it != nodes_.end())
...
template <class NodeId, class NodeInfo, class EdgeInfo>
inline Graph<NodeId, NodeInfo, EdgeInfo>& Graph<NodeId, NodeInfo, EdgeInfo>::unhide_edge(const NodeId& from,
                                                                                         const NodeId& to) {
	auto it = find_edge(from, to);
	if (it != edges_.end())
		it->second.hidden_ = false;
...
template <class NodeId, class NodeInfo, class EdgeInfo>
inline Graph<NodeId, NodeInfo, EdgeInfo>& Graph<NodeId, NodeInfo, EdgeInfo>::remove_node(const NodeId& node) {
	auto it = nodes_.find(node);
	if (it != nodes_.end())
		remove_node(it);
	return *this;
...
template <class NodeId, class NodeInfo, class EdgeInfo>
inline Graph<NodeId, NodeInfo, EdgeInfo>& Graph<NodeId, NodeInfo, EdgeInfo>::remove_all_nodes() {
	nodes_.clear();
	edges_.clear();
	return *this;
}
...
template <class NodeId, class NodeInfo, class EdgeInfo>
inline Graph<NodeId, NodeInfo, EdgeInfo>& Graph<NodeId, NodeInfo, EdgeInfo>::remove_edge(const NodeId& from,
                                                                                         const NodeId& to) {
	auto it = find_edge(from, to);
	if (it != edges_.end())
		remove_edge(it);
...
template <class NodeId, class NodeInfo, class EdgeInfo>
inline Graph<NodeId, NodeInfo, EdgeInfo>& Graph<NodeId, NodeInfo, EdgeInfo>::remove_all_edges() {
