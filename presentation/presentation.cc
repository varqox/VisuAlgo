...
void add_column_view(Slide& slide, double ratio, const string& cpp_source_title, const string& cpp_source,
                     Elems&&... elems) {
	// First column
...
void add_graphs(LatexPresentation& pres) {
	...
	}
	{
		...
		sb.add_elem(g);
		g.add_edge(0, 1).add_edge(1, 0).add_edge(1, 2).add_edge(2, 3).add_edge(3, 4).add_edge(4, 5);
		...
		SourceCode code;
		code.set_lang("C++");
		code.set_code(R"=(SlideBuilder sb;)="
		              "\n"
	...
	}
	{
		...
		code.set_code(R"(q.emplace(source);)"
		              "\n"
		              R"(dist[source] = 0;)"
		              "\n"
...
void add_arrays(LatexPresentation& pres) {
	...
	}
	{
		...
		SourceCode code;
		code.set_lang("C++").set_code(R"=(Array2D<int> arr("Array2D", 5, 10);)="
		                              "\n"
		                              R"=()="
		                              "\n"
		                              R"=(for (int i = 0; i < 5; ++i))="
	...
	}
	{
		...
		code.set_code(R"(for (int i = 2; i < N; ++i) {)"
		              "\n"
		              R"(	if (is_prime[i]) {)"
		              "\n"
		              R"(		wielo.set(i);)"
